...
function success(dom) { $(dom).attr("class", "").next("p").attr("class", "").html('<img src="http://sf.panli.com/FrontEnd/images20090801/Register/gou.gif" />'); }
...
function nicknameblur(dom) {
    ...
    if (nickname.length <= 0) { $(dom).attr("class", "").val("").next("p").attr("class", "").text("用户名也可以作为您登录Panli的帐号，大家以后就这样称呼您"); nicknamechecked = false; }
    else {
