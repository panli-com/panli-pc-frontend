$(function() {
    ...
    $('#rrSubmitBtn').click(function() {
        ...
        $.ajax({
            ...
            data: '{"alipayAccount":"' + aAcount + '","userInfo":"' + ur + '","price":' + amount + ',"vcode":"' + code + '"}',
            timeout: 15000,
            ...
            success: function(r) {
                if (r.d == 'Success') {
                    $('#rrMainPanel').html('<div class="succeed"><h2>恭喜您！成功提交了代充值申请！</h2><p>本次申请代充值<span>' + $('.rrAmount:visible').text() + '</span>元，我们将在1个工作日内进行审核，请耐心等待！</p><p>本次申请扣除<span>' + $('#rrsFee').text() + '</span>元手续费，消耗<span>' + $('#rrsPoint').text() + '</span>点积分！</p><ul><li><a href="/mypanli/ReplaceRecharge/Record.aspx">查看代充记录</a></li><li><a href="/">返回Panli首页</a></li></ul></div>');
                    return;
                }
                if (r.d == 'BalanceLack') {
