...
function deleteFavorite(id) {
    if (confirm("您确定要删除收藏吗？")) {
        $.ajax({
            ...
            success: function(res) { if (res.d != "失败") { ShowTag(res.d); $("#tr_favor_" + id).remove(); IsNeedReload($(".sc table tr").length); if ($(".sc table tr").length < 1) { window.location = ToPrevPage(); } } else { alert("删除失败了！"); } }
        });
    }
}
//取消选中收藏
function deleteSelectFavorite() {
    ...
    if (confirm("您确定要删除这些商品吗？")) {
        $.ajax({
            ...
            timeout: 10000,
            error: function() { alert("网络错误，请稍后再试！"); },
            success: function(res) { if (res.d != "失败") { ShowTag(res.d); var ids = id.split(","); for (i = 0; i < ids.length - 1; i++) $("#tr_favor_" + ids[i]).remove(); IsNeedReload($(".sc table tr").length); if ($(".sc table tr").length < 1) { window.location = ToPrevPage(); } } else { alert("删除失败了！"); } }
        });
    }
}
...
function tagShow(isShow) {
    if (isShow) {
        ...
        $("#txtTagName").keydown(function(e) { if (e.keyCode == 13) { $(".sc_dialog .o").click(); return false; } });
        $("#txtTagName").focus();
    }
    else {
...
function EditUserTag(tagNumber, oldTag, favoriteCount) {
    ...
    if (vt == 2) {
        $("#p_Tag_m").text("每个标签的字数限14个字符，请重新输入！").addClass("red");
        return;
    }
    else if (vt == 3) {
...
function AddUserTag() {
    ...
    if (vt == 2) {
        ...
        $(".sc_dialog .o").attr("disabled", true);
        $.ajax({
            type: "POST",
            ...
            data: "{tagName:'" + ReplaceD(tagName) + "'}",
            timeout: 10000,
            error: function() { alert("网络错误，请稍后再试！"); $(".sc_dialog .o").attr("disabled", false); },
            success: function(res) {
                if (res.d) {
                    $(".sc_dialog .o").attr("disabled", false);
                    if (flag != 1) flag = 2;
...
function ValidateTagName(tagName) {
    ...
    if (!regtest.test(tagName)) {
        ...
        var res;
        while ((res = regEng.exec(tagName)) != null)
            nicknamelength++;
        while ((res = regCn.exec(tagName)) != null)
            nicknamelength += 2;
...
function AddFavoriteTag() {
    if (fId > 0) {
        ...
        for (i = 0; i < txtTags.length; i++) {
            ...
            if (vc == 2) {
                $("#p_Tag").text("每个标签的字数限14个字符，请重新输入！").addClass("red");
                return;
            }
            else if (vc == 3) {
...
function ShowTag(res) {
    ...
    if (tags.length > 0) {
        $.each(tags, function(i, data) {
            if (queryTag == "" && i == 0) {
                str += "<span class=\"stay\"><a href=\"/mypanli/Favorite.aspx\">" + data.TagName + "</a>(" + data.FavoriteCount + ")</span>";
            } else if (i == 0) {
                str += "<span class=\"stay\"><a href=\"/mypanli/Favorite.aspx\">" + data.TagName + "</a>(" + data.FavoriteCount + ")</span>";

            }
            else if (queryTag == ReplaceD(data.TagName)) {
                str += "<span class=\"stay\"><a href=\"/mypanli/Favorite.aspx?tag=" + ReplaceA(encodeURI(data.TagName)) + "\">" + data.TagName + "</a>(" + data.FavoriteCount + ")</span>";

            }
            else {
                str += "<span><a href=\"/mypanli/Favorite.aspx?tag=" + ReplaceA(encodeURI(data.TagName)) + "\">" + data.TagName + "</a>(" + data.FavoriteCount + ")</span>";
            }
        });
    }
    $("#Tag_show").html(str);
