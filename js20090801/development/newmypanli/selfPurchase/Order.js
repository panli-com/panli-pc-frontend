$(function() {
    ...
    $('#selfUrl').focus(function() {
        //                if ($('#selfUrl').val() == '请输入商品网址！' || $('#selfUrl').val() == '您输入的链接地址不正确，请核实后再填写！')
        //                    $('#selfUrl').removeClass('error').val('')
        $("#urlError").hide();
        $(this).select();
        return false;
    }).keydown(function(e) {
    ...
    $('#spinputsubmit').click(function() {
        ...
        $.ajax({
            ...
            dataType: "json",
            contentType: "application/json;utf-8",
            data: '{"name":"' + $('#spname').val() + '","remark":"' + $('#spremark').val() + '"}',
            timeout: 15000,
    ...
    function addInputPanelInit() {
        $('#spremark,#spname').attr('class', 'red');
        $('#spname').val('请填写商品名称');
